...
def _execute_steps(steps_config: list):
    ...
    for i, step in enumerate(steps_config):
        ...
        logging.info(
            f"--- Running Step {i+1}/{len(steps_config)}: Calling {module_name}.{function_name} ---"
        )
        try:
            ...
            logging.error(
                f"!!! Step {i+1} ({module_name}.{function_name}) failed: {e} !!!"
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if pipeline_to_run == "daily":
            ...
            print(
                f"Error: Unknown argument '{pipeline_to_run}'. Please use 'daily' or 'weekly'."
            )
    else:
